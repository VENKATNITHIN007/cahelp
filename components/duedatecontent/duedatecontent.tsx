...
export default function DueDateContent() {
  ...
  const period: Period = ["all", "today", "week", "month"].includes(rawPeriod)
    ? (rawPeriod as Period)
    : "all";
  const rawFilter = searchParams?.get("filter") ?? undefined;
  const specialFilter: SpecialFilter =
    rawFilter === "urgent" || rawFilter === "overdue"
      ? (rawFilter as SpecialFilter)
      : undefined;
  ...
  const {
    ...
    status,
    period:
      period === "all" ? undefined : (period as "today" | "week" | "month"),
  ...
  function updateQueryParams(next: {
    ...
    if (Object.prototype.hasOwnProperty.call(next, "period")) {
      if (next.period && next.period !== "all")
    ...
    router.replace(
      `/app/duedates${params.toString() ? `?${params.toString()}` : ""}`
    );
  }
  ...
  const filteredGroups = useMemo(() => {
    ...
    return grouped
      .map((g: any) => {
        const dues = (g.dues || []).filter((d: any) =>
          labelFilter ? String(d.label) === labelFilter : true
        );
        return { ...g, dues };
      })
  ...
  const toggleContactAndScroll = (id: string) => {
    ...
    setTimeout(() => {
      const el =
        rowRefs.current[willOpen ? `${id}-panel` : id] ?? rowRefs.current[id];
      if (el)
  ...
  const activeHeading = (() => {
    if (specialFilter)
      return `${specialFilter === "urgent" ? "Urgent" : "Overdue"} due dates`;
  ...
  if (isError)
    return <p className="p-4 text-red-600">Failed to load due dates</p>;

  if (!filteredGroups || filteredGroups.length === 0) {
    const emptyTitle = specialFilter
      ? `No ${specialFilter === "urgent" ? "urgent" : "overdue"} due dates`
      : labelFilter
      ? `No ${String(labelFilter).toUpperCase()} due dates`
      : period !== "all"
      ? `No due dates for ${
          period === "today"
            ? "Today"
    ...
    return (
      <div className="p-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-2">
              <label className="text-sm text-muted hidden sm:block">
                Period
              </label>
              <select
                value={period}
                onChange={(e) => applyPeriodOnly(e.target.value as Period)}
            ...
            <div className="flex items-center gap-2">
              <Button
                size="sm"
                variant={specialFilter === "urgent" ? "default" : "outline"}
                onClick={() =>
                  applySpecialOnly(
                    specialFilter === "urgent" ? undefined : "urgent"
                  )
                }
              >
                Urgent
              ...
              <Button
                size="sm"
                variant={specialFilter === "overdue" ? "default" : "outline"}
                onClick={() =>
                  applySpecialOnly(
                    specialFilter === "overdue" ? undefined : "overdue"
                  )
                }
              >
                Overdue
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                value={labelFilter ?? ""}
                onChange={(e) => applyLabelOnly(e.target.value || undefined)}
                className="rounded border px-2 py-1 text-sm"
              >
        ...
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-center text-sm text-muted max-w-xs mt-2">
            Try clearing filters or add a new due date.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="p-4">
      ...
      {filteredGroups.map((group) => (
        <section
          key={`${group.year}-${String(group.month).padStart(2, "0")}`}
          className="mb-8"
        >
          ...
          <div className="grid gap-3 md:hidden">
            {group.dues.map((d: any) => {
              ...
              const message = `Hello ${
                d.clientName ?? "Client"
              },\n\nPlease share the details for "${
                d.title
              }" which is due on ${formatFriendly(d.date)}.\n\nThank you.`;
              return (
                <div
                  key={d._id}
                  ref={(el) => {
                    rowRefs.current[d._id] = el;
                  }}
                >
                    ...
                    <CardContent className="p-3 space-y-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            onStatusChange(d._id, e.target.value)
                          }
                      ...
                      <Button
                        size="sm"
                        variant={
                          openContactId === d._id ? "default" : "outline"
                        }
                  ...
                  {openContactId === d._id && (
                    <div
                      ref={(el) => {
                        rowRefs.current[`${d._id}-panel`] = el;
                      }}
                    ...
                    >
                      <textarea
                        readOnly
                        rows={4}
                        className="w-full rounded-md border p-2 text-sm whitespace-pre-wrap resize-none"
                        value={message}
                      />
                      <div className="flex gap-2 mt-2">
                        <Button
                          size="sm"
                          onClick={() => copyContactMessage(message)}
                ...
                <tbody>
                  {group.dues.map((d: any) => {
                    ...
                    const message = `Hello ${
                      ...
                      d.title
                    }" which is due on ${formatFriendly(
                      d.date
                    )}.\n\nThank you.`;
                    return (
                        ...
                        >
                          <td className="p-3 max-w-[24rem]">
                            <div className="flex items-center gap-2">
                              ...
                              {d.label && (
                                <LabelBadge
                                  text={String(d.label).toUpperCase()}
                                />
                              )}
                              {recurrenceLetter(d.recurrence) && (
                                <RecurrenceBadge
                                  text={recurrenceLetter(d.recurrence)!}
                                />
                              )}
                            </div>
                          </td>
